...
function webmcpFindings({ observed, headers, tools }) {
  ...
  const findings = [
    ...
    finding("webmcp", "model-context", observed.modelContextAvailable ? "pass" : "warning", "API WebMCP no navegador", observed.modelContextAvailable ? "document.modelContext está disponível neste ambiente." : "document.modelContext não foi exposto pelo navegador usado no scan.", observed.modelContextAvailable ? null : "Mantenha feature detection; a disponibilidade depende do navegador configurado."),
    ...
    finding("webmcp", "permissions-policy", headers["permissions-policy"] ? "pass" : "warning", "Política de permissões", headers["permissions-policy"] ? `Permissions-Policy: ${headers["permissions-policy"]}` : "O cabeçalho Permissions-Policy não foi observado.", headers["permissions-policy"] ? null : "Declare apenas capacidades e origens realmente necessárias.")
  ];
