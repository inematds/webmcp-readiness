import { schemaIsObject } from "./text-signals.mjs";
...
function webmcpFindings({ observed, headers, tools }) {
  ...
  const validSchemas = observed.imperativeTools.filter((tool) => schemaIsObject(tool.inputSchema));
...
function seoFindings({ observed, headers, files, statusCode }) {
  ...
  return [
    ...
    finding("seo", "sitemap", files.sitemap.found && files.sitemap.urlCount && (files.sitemap.lastModifiedCount / files.sitemap.urlCount) >= 0.5 ? "pass" : "warning", "Sitemap", files.sitemap.found ? `${files.sitemap.urlCount} URL(s) em ${files.sitemap.sitemapCount ?? 1} sitemap(s); ${files.sitemap.lastModifiedCount} com lastmod.` : "sitemap.xml não encontrado.", files.sitemap.found && files.sitemap.urlCount && (files.sitemap.lastModifiedCount / files.sitemap.urlCount) < 0.5 ? "Informe lastmod real nas URLs do sitemap; sem ele os rastreadores não sabem o que mudou." : "Publique URLs canônicas no sitemap e mantenha lastmod correto."),
