...
export async function analyzeReadiness(rawUrl) {
  ...
  try {
    ...
    const observed = await page.evaluate(async () => {
      ...
      const hasVisibleAuthor = /\b(autor|autora|por|escrito por|revisado por|author|written by)\b/i.test(bodyText);
      const hasDate = Boolean(document.querySelector("time[datetime], meta[property='article:published_time'], meta[property='article:modified_time']")) || /\b(?:atualizado|publicado|updated|published)\b/i.test(bodyText);
      ...
      return {
        ...
        content: {
          ...
          definitions: [...document.querySelectorAll("p,dd")].filter((node) => /\b(é|significa|refere-se|define-se|is defined as|means)\b/i.test(node.textContent || "")).length,
          questionHeadings: questionHeadings.length,
          directAnswers,
          hasAuthor: Boolean(authorMeta || document.querySelector("[rel='author'], .author, [itemprop='author']") || hasVisibleAuthor),
          hasDate,
    ...
    for (const tool of observed.imperativeTools) imperativeByName.set(tool.name, tool);
